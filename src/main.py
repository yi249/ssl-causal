...
def main(args: Hparams):
    ...
    def init_bias(m):
        if type(m) == torch.nn.Conv2d and m.bias is not None:
    ...
    # setup optimizer
    optimizer, optimizer_adv, optimizer_cls, scheduler, scheduler_adv, scheduler_cls = setup_optimizer(args, model)
    ...
    if ckpt is not None:
        ...
        scheduler_adv = torch.optim.lr_scheduler.LambdaLR(
            optimizer_adv, lr_lambda=lambda x: x * 0 + 1
        )
